// This file is a modified version of lib/std/zig/Ast/Render.zig
// Upstream version used: zig-0.15.1
...
fn renderZloppyUnused(r: *Render, token: Ast.TokenIndex) !void {
    const name = tokenSliceForRender(r.tree, token);
    try r.ais.print("_ = {s}; {s} unused var {s}\n", .{ name, zloppy_comment, name });
    r.patches.rendered_comments += 1;
}

fn renderZloppyIgnoredCall(r: *Render, node: Ast.Node.Index) !void {
    try r.ais.print("_ = ", .{});
    try renderExpression(r, node, .none);
    try r.ais.print("; {s} ignored call return value\n", .{zloppy_comment});
    r.patches.rendered_comments += 1;
}
...
fn renderZloppyCommentedOutLines(
    ...
    const from_tok_pos = r.tree.tokens.items(.start)[from_token];
    const from_pos =
        if (mem.lastIndexOf(u8, r.tree.source[0..from_tok_pos], "\n")) |pos| pos + 1 else 0;

    // get last new line before to_token (rbrace of block containing commented out statements)
    std.debug.assert(r.tree.tokens.items(.tag)[to_token] == .r_brace);
    const to_tok_pos = r.tree.tokens.items(.start)[to_token];
    const to_pos =
        if (mem.lastIndexOf(u8, r.tree.source[0..to_tok_pos], "\n")) |pos| pos else unreachable;

    // comment out all lines between from_pos and to_pos
    var it = mem.splitScalar(u8, r.tree.source[from_pos..to_pos], '\n');
    while (it.next()) |line| {
...
pub fn renderTreeWithPatches(gpa: Allocator, w: *Writer, tree: Ast, patches: *Patches) Error!void {
    ...
    switch (tree.nodeTag(decl)) {
        .fn_decl => {
            ...
            if (r.fixups.gut_functions.contains(decl)) {
                ...
                return finishRenderBlock(r, body_node, 0, .{}, statements, space);
            } else {
...
fn renderExpression(r: *Render, node: Ast.Node.Index, space: Space) Error!void {
    ...
    // If patches are anchored on this node, and this node is not a block nor a destructure, it
    ...
    if (!nodeIsBlock(tag) and tag != .assign_destructure) {
        if (r.patches.get(r.tree.nodeMainToken(node))) |tok_patches| {
            try r.ais.writeAll("{");
            ...
            for (tok_patches) |patch| {
                switch (patch) {
                    .unused_var => |token| {
                        try renderZloppyUnused(r, token);
                    },
                    else => {},
                }
            }
            try renderExpressionInner(r, node, .semicolon);
            r.ais.popIndent();
            try r.ais.writeAll("}\n");
            return;
        }
    ...
    try renderExpressionInner(r, node, space);

    // Destructure nodes will have patches related to all variables attached here
...
fn renderExpressionInner(r: *Render, node: Ast.Node.Index, space: Space) Error!void {
    const tree = r.tree;
    const ais = r.ais;
...
fn renderVarDecl(
    ...
    try renderVarDeclInner(r, var_decl, ignore_comptime_token, space);

    // Destructures are taken care of in `renderExpression`
    if (space == .space or space == .comma_space)
        return;

    if (r.patches.get(var_decl.ast.mut_token)) |tok_patches| {
        for (tok_patches) |patch| {
            switch (patch) {
                .unused_var => |token| try renderZloppyUnused(r, token),
                else => {},
            }
        }
    }
...
fn renderThenElse(
    ...
    const patches = r.patches.get(tree.nodeMainToken(then_expr));
    const then_expr_is_block = nodeIsBlock(tree.nodeTag(then_expr)) or patches != null;
...
fn renderBlock(
    ...
    var ignored_fn_calls: std.ArrayListUnmanaged(Ast.TokenIndex) = .{};
    defer ignored_fn_calls.deinit(r.gpa);

    if (r.patches.get(lbrace)) |tok_patches| {
        for (tok_patches) |patch| {
            switch (patch) {
                .unused_var => |token| {
                    if (!lbrace_rendered) {
                ...
                .ignore_ret_val => |token| {
                    try ignored_fn_calls.append(r.gpa, token);
                },
            }
        }
    }
    ...
    if (!lbrace_rendered)
        try renderToken(r, lbrace, if (statements.len == 0) .none else .newline);
...
fn finishRenderBlock(
    r: *Render,
    block_node: Ast.Node.Index,
    first_unreachable_stmt: Ast.TokenIndex,
    ignored_fn_calls: std.ArrayListUnmanaged(Ast.TokenIndex),
    statements: []const Ast.Node.Index,
    space: Space,
) Error!void {
    const tree = r.tree;
    ...
    for (statements, 0..) |stmt, i| {
        ...
        const main_token = tree.nodeMainToken(stmt);
        ...
        switch (tree.nodeTag(stmt)) {
            ...
            => try renderVarDecl(r, tree.fullVarDecl(stmt).?, false, .semicolon),
                ...
                for (ignored_fn_calls.items) |token| {
                    if (main_token == token) {
                        try renderZloppyIgnoredCall(r, stmt);
                        break;
                    }
                } else {
