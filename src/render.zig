...
// Upstream version used: zig-0.12.0-dev.890+8c6b0271c
...
pub fn renderTreeWithPatches(buffer: *std.ArrayList(u8), tree: Ast, patches: *Patches) Error!void {
    ...
    var auto_indenting_stream = Ais{
        ...
        // Patches are stored in Ais to limit the needed modifications on upstream render.zig
        .patches = patches,
    };
...
fn renderExpression(gpa: Allocator, ais: *Ais, tree: Ast, node: Ast.Node.Index, space: Space) Error!void {
    ...
    try renderExpressionInner(gpa, ais, tree, node, space);
...
fn renderVarDecl(
    ...
    try renderVarDeclInner(gpa, ais, tree, var_decl, ignore_comptime_token, space);
...
fn renderThenElse(
    ...
    const patches = ais.patches.get(tree.nodes.items(.main_token)[then_expr]);
