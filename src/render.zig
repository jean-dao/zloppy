...
// Upstream version used: zig-0.14.0
...
fn renderExpression(r: *Render, node: Ast.Node.Index, space: Space) Error!void {
    ...
    if (!nodeIsBlock(tag) and tag != .assign_destructure) {
        if (r.patches.get(r.tree.nodes.items(.main_token)[node])) |tok_patches| {
...
fn renderThenElse(
    ...
    const patches = r.patches.get(tree.nodes.items(.main_token)[then_expr]);
    const then_expr_is_block = nodeIsBlock(node_tags[then_expr]) or patches != null;
...
fn finishRenderBlock(
    r: *Render,
    block_node: Ast.Node.Index,
    first_unreachable_stmt: Ast.Node.Index,
